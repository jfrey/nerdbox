...
func (vmc *vmcontext) AddPmemImageConfig(config vm.PmemImageConfig) error {
	...
	if ret != 0 {
		return fmt.Errorf("krun_add_pmem_image_config failed: %d", ret)
	}
	return nil
}
...
type libkrun struct {
	...
	AddPmemImageConfig func(ctxID uint32, config unsafe.Pointer) int32                                        `C:"krun_add_pmem_image_config" optional:"true"`
